...
function moveSnake() {
  ...
  snake.push(new Coordinate(snakeHead.x, snakeHead.y));
...
function checkGameOver() {
  ...
  const isSelfCollision = snake.filter((snakePart: Coordinate) => snakePart.x === snakeHead.x && snakePart.y === snakeHead.y).length === 2;
